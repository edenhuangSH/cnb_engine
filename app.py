"""
Amazon Compensation Analytics Dashboard
...
st.set_page_config(
    page_title="Amazon Compensation Analytics",
    page_icon="📦",
    layout="wide",
)
...
<div class="amazon-header">
    <svg class="amazon-logo" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
        <rect width="100" height="100" rx="12" fill="#232F3E"/>
        <text x="50" y="55" text-anchor="middle" fill="#FF9900" font-size="40" font-weight="bold" font-family="Arial">a</text>
        <path d="M30 68 Q50 78 70 68" stroke="#FF9900" stroke-width="3" fill="none" stroke-linecap="round"/>
    </svg>
    <div>
        <p class="amazon-title">Compensation & Benefits Analytics</p>
    </div>
</div>
""", unsafe_allow_html=True)
st.caption("Gender Pay Gap Analysis | Delivered by <span class='mercer-badge'>Mercer</span>",
           unsafe_allow_html=True)
...
with tab1:
    ...
    with col_right:
        st.subheader("Average Compensation by Level & Gender")
        ...
        fig = px.bar(
            ...
            barmode="group",
            color_discrete_map={"M": "#4A90D9", "F": "#E85D75"},
            labels={"job_level": "Level (L4-L12)", "total_compensation": "Avg Total Comp", "gender": "Gender"},
        )
        fig.update_layout(height=400)
        st.plotly_chart(fig, use_container_width=True)

    st.subheader("Gender Representation by Level")
    ...
    fig = px.bar(
        ...
        color_discrete_map={"M": "#4A90D9", "F": "#E85D75"},
        labels={"job_level": "Level (L4-L12)", "pct": "% of Employees", "gender": "Gender"},
    )
...
with tab2:
    ...
    with col1:
        ...
        fig = px.bar(
            ...
            barmode="group", color_discrete_map={"M": "#4A90D9", "F": "#E85D75"},
        )
...
with tab3:
    ...
    budget_pct = st.slider("Correction Budget (% of full correction)", 0, 100, 100, 5)
    ...
    if len(plan.corrections) > 0:
        ...
        with col_right:
            ...
            fig = px.histogram(plan.corrections, x="raise_pct", nbins=30, labels={"raise_pct": "Raise %"})
            fig.update_layout(height=350)
            st.plotly_chart(fig, use_container_width=True)
        ...
        st.download_button("📥 Download Full Correction Report (CSV)", csv,
                           "amazon_pay_gap_corrections.csv", "text/csv")
...
with tab4:
    ...
    st.download_button("📥 Download Employee Data (CSV)",
                       ...
                       "amazon_employee_data.csv", "text/csv")
